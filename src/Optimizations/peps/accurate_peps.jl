...
using ..ITensorNetworks: PEPS, inner_network, inner_networks, flatten, rayleigh_quotient, MPSTensor, abstract_network
...
function loss_grad_wrap(peps::PEPS, Hs::Array, ::typeof(abstract_network), tensortype; kwargs...)
  function loss(peps::PEPS)
    peps_bra = addtags(linkinds, peps, "bra")
    peps_ket = addtags(linkinds, peps, "ket")
    sites = commoninds(peps_bra, peps_ket)
    ...
    network_list = vcat(network_H, [network_inner])
    variables = flatten([peps_bra, peps_ket, peps_ket_ham])
    inners = batch_tensor_contraction(abstract_network, tensortype, network_list, variables...)
    return rayleigh_quotient(inners)
  end
  loss_w_grad(peps::PEPS) = loss(peps), gradient(loss, peps)[1]
  return loss_w_grad
...
function gradient_descent(peps::PEPS, Hs::Array, ::typeof(abstract_network), tensortype; stepsize::Float64, num_sweeps::Int, kwargs...)
